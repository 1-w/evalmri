define(function () {
      ...
      for (x = 0; x < width; x += 1) {
        ...
        for (i = 0; i < num_images; i += 1) {
          ...
          if (y < image.height && x < image.width) {
            ...
            //Red
            target_data[pixel] =
              target_data[pixel] * alpha + image_data[current] * alphas[i];

            //Green
            target_data[pixel + 1] =
              target_data[pixel + 1] * alpha +
              image_data[current + 1] * alphas[i];

            //Blue
            target_data[pixel + 2] =
              target_data[pixel + 2] * alpha +
  ...
  var ret = function (options, callback) {
    ...
    overlay_volume.getSliceImage = function (
      slice,
      zoom,
      contrast,
      brightness
      ...
      slices.forEach(function (slice, i) {
        ...
        var target_image = image_creation_context.createImageData(
          max_width,
          max_height
        );
        ...
        var time_offset = header.time
          ? volume.current_time * header.time.offset
          : 0;
